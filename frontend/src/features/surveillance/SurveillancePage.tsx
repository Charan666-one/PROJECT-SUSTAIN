...
const ANOMALY_LABEL: Record<string, string> = {
  on_track: "On track", aggravation: "Homeopathic aggravation", plateau: "Plateau",
  non_response: "No response", relapse: "Relapse", worsening: "Worsening",
};
...
export default function SurveillancePage() {
  ...
  const close = async (visitId: string) => { await surveillanceApi.close(visitId); load(); };
  ...
  return (
    <div>
      ...
      {episodes.length === 0 && <div className="card muted">No episodes yet. Approve a prescription to begin surveillance.</div>}

      {episodes.map((e) => (
        <div key={e.visit_id} className={`card episode ${e.severity}`}>
          <div className="row between">
            <div><strong style={{ fontSize: "1.05rem" }}>{e.patient_name}</strong>
              <span className="muted"> · {e.chief_complaint || "—"}</span></div>
          ...
          <div className="row" style={{ marginTop: ".75rem" }}>
            {e.suggest_re_evaluation && (
              <button className="btn sm" onClick={() => navigate(`/consultation/${e.visit_id}`)}>Re-evaluate remedy →</button>
            )}
